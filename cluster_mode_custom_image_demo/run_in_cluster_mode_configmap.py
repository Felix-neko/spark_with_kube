...
Использует ConfigMap для передачи app.py вместо HTTP-сервера.

Подход:
1. Создаётся ConfigMap с содержимым app.py
2. ConfigMap монтируется в под драйвера
3. Spark запускает файл из примонтированного ConfigMap
4. После завершения ConfigMap удаляется
"""
...
class SparkSubmitThread(threading.Thread):
    ...
    def __init__(self, app_name: str, log_host: str, log_port: int, configmap_name: str):
    ...
    def _run_spark_submit(self):
        cmd = [
            ...
            # Монтируем ConfigMap в под драйвера
            "--conf", f"spark.kubernetes.driver.volumes.configMap.{self.configmap_name}.mount.path=/opt/spark/work-dir/app",
            "--conf", f"spark.kubernetes.driver.volumes.configMap.{self.configmap_name}.options.name={self.configmap_name}",
            ...
            # Указываем путь к файлу внутри контейнера (примонтированный из ConfigMap)
            "local:///opt/spark/work-dir/app/app.py",
        ]
...
def create_configmap(name: str, app_file: Path) -> bool:
    """Создаёт ConfigMap с содержимым app.py."""
    log_info(f"Создание ConfigMap '{name}'...")
    
    result = subprocess.run(
        [
            "kubectl", "create", "configmap", name,
            f"--from-file=app.py={app_file}",
            "-n", NAMESPACE,
        ],
        capture_output=True,
        text=True
    ...
    if result.returncode != 0:
        log_error(f"Не удалось создать ConfigMap: {result.stderr}")
        return False
    
    log_info(f"✓ ConfigMap '{name}' создан")
    return True


def delete_configmap(name: str):
    """Удаляет ConfigMap."""
    log_info(f"Удаление ConfigMap '{name}'...")
    ...
    if result.returncode == 0:
        log_info(f"✓ ConfigMap '{name}' удалён")
    else:
        log_error(f"Не удалось удалить ConfigMap: {result.stderr}")
...
def main():
    ...
    # Создаём ConfigMap с app.py
    if not create_configmap(configmap_name, app_file):
        return 1
